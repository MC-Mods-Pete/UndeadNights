...
public class TntIgniteAndThrowGoal extends Goal {
	...
	public boolean canUse() {
		...
		return this.demolitionZombie.distanceToSqr(target) < 18.0;
	}
	...
	public void tick() {
		...
		if (tntCoolDown <= 0) {
			if (this.target != null) {
				if ((this.demolitionZombie.distanceToSqr(this.target) < 18.0)
				&& (this.demolitionZombie.distanceToSqr(this.target) > 5.0)
					...
					final Random random = this.demolitionZombie.level.random;
					...
					Vec3 vec3 = (new Vec3(pX, pY, pZ)).normalize().add(triangle(random,0.0D, 0.0172275D * (double)pInaccuracy), triangle(random,0.0D, 0.0172275D * (double)pInaccuracy), triangle(random,0.0D, 0.0172275D * (double)pInaccuracy)).scale((double)pVelocity);
