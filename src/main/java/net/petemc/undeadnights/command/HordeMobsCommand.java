...
public class HordeMobsCommand {
    ...
    private int removeMobs(CommandSourceStack source) throws CommandSyntaxException {
        int count = 0;
        Objects.requireNonNull(source.getEntity()).sendSystemMessage(Component.translatable("message.undeadnights.command_remove_horde_mobs"));
    ...
    private int blockBreaking(CommandSourceStack source, boolean value, int tier) throws CommandSyntaxException {
        if (tier == 0) {
            ...
            Objects.requireNonNull(source.getEntity())
                    .sendSystemMessage(Component.literal("Block breaking for Horde and Elite Zombies " + (value ? "enabled" : "disabled")));
        } else {
            hordeZombiesBlockBreakingTier = tier;
            Objects.requireNonNull(source.getEntity())
                    .sendSystemMessage(Component.literal("Setting block breaking tier " + tier));
        }
        return 0;
    }
}