...
public class HordeMobsCommand {
    ...
    private int removeMobs(CommandSourceStack source) throws CommandSyntaxException {
        int count = 0;
        Objects.requireNonNull(source.getEntity()).sendMessage(Component.nullToEmpty("Removing all horde mobs from this world."), source.getEntity().getUUID());
    ...
    private int printConfig(CommandSourceStack source) throws CommandSyntaxException {
        if (HordeConfig.getReadingConfigFailed()) {
            Objects.requireNonNull(source.getEntity())
                    .sendMessage(Component.nullToEmpty("Reading the horde mob config failed!\nSpawning 15 default horde zombies instead.").copy().withStyle(ChatFormatting.YELLOW), source.getEntity().getUUID());
            return 0;
        }
